...
// asyncBlockToFrame converts all blocks to respective string frame in order.
// assumes that blocks coming at least have 1 row.
// returns false if channel is closed initially.
func asyncBlockToFrame(blockStream <-chan *data.Block) (<-chan [][]string, []*column.CHColumn, func([][]string), bool) {
