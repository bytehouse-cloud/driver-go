...
func Test_getInsertArgsTable(t *testing.T) {
	type args struct {
		query string
		args  []driver.Value
	}
	tests := []struct {
		...
		{
			name: "Should return right array for valid query",
			args: args{
				query: "INSERT INTO mama VALUES (?), (?)",
				args:  []driver.Value{1, 3},
			},
			want:    [][]interface{}{{1}, {3}},
			wantErr: false,
		},
		...
		},
		{
			...
			args: args{
				query: "INSERT INTO mama VALUES (?, ?, ?), (?, ?, ?)",
				args:  []driver.Value{1, 2, 3, 4, "pa", "pa"},
			},
			want:    [][]interface{}{{1, 2, 3}, {4, "pa", "pa"}},
			wantErr: false,
		},
		{
			name: "Should return error for invalid query",
			args: args{
				query: "INSERT INTO mama VALUES (?, ?, ?), (?)",
				args:  []driver.Value{1, 2, 3, 4},
			},
		...
		},
		{
			...
			args: args{
				query: "INSERT INTO mama VALUES (?, ?, ?), (?, ?, ?)",
				args:  []driver.Value{1, 2, 3, 4, 5, 6, 7, 8},
			},
			want:    nil,
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := getInsertArgsTable(tt.args.query, tt.args.args)
			if (err != nil) != tt.wantErr {
				t.Errorf("getInsertArgsTable() error = %v, wantExecErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("getInsertArgsTable() got = %v, want %v", got, tt.want)
			}
		})
	}
}
